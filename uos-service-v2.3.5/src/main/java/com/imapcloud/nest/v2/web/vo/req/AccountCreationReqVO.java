...
@ApiModel("账号新建信息")
@Data
public class AccountCreationReqVO implements Serializable {

    @ApiModelProperty(value = "用户名", example = "dev@geoai")
    @Pattern(regexp = "^[A-Za-z\\d]{6,18}$", message = "{geoai_uos_limited_6_18_user_name}")
    @NotBlank(message = "{geoai_uos_cannot_empty_username}")
    private String account;

    @ApiModelProperty(value = "密码", example = "dev@geoai")
    ...
    @ApiModelProperty(value = "角色ID列表", example = "")
    @NotNull(message = "必须至少指定一个角色")
    @Size(min = 1, message = "必须至少指定一个角色")
