 ...
 * @Author Carnival
 */
@ApiModel("Mqtt代理地址新增信息")
@Data
public class UosMqttCreationReqVO implements Serializable {

    @ApiModelProperty(value = "代理地址名称", position = 1, required = true, example = "中科云图")
    @NotNull(message = "{geoai_uos_cannot_empty_mqtt}")
    @Length(min = 2, max = 16, message = "{geoai_uos_limited_2_16_mqtt}")
    ...
    @Length(min = 0, max = 200, message = "{geoai_uos_cannot_exceed_200_char_out_mqtt}")
    ...
    @ApiModelProperty(value = "管理账号", position = 4, required = true, example = "zkyt")
    @NotNull(message = "{geoai_uos_cannot_empty_useraccount}")
    @Length(min = 1, max = 80, message = "{geoai_uos_cannot_exceed_80_password}")
    private String account;

    @ApiModelProperty(value = "管理密码", position = 5, required = true, example = "123456")
