...
@Slf4j
@Component
@DownloadAnnotation(key = "download_airspace_application")
public class AirspaceApplicationDownloadHandler extends AbstractDownloadHandler implements DownloadHandler {
    ...
    protected boolean checkParam(HandlerIn handlerIn) {
        ...
        AirspaceApplicationDownloadHandler.Bean bean = toBean(param);
    ...
    protected void realExportImpl(HandlerIn handlerIn, HttpServletResponse response) throws Exception {
        Bean bean = toBean(handlerIn.getParam());
        airspaceManageService.exportAirspace(response, bean.airspaceId);
    }


    private AirspaceApplicationDownloadHandler.Bean toBean(String param) {
        return JSONUtil.toBean(param, AirspaceApplicationDownloadHandler.Bean.class);
    }
